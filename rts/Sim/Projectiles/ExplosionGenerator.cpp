...
bool CCustomExplosionGenerator::Load(CExplosionGeneratorHandler* handler, const char* tag)
{
	...
	for (auto& spawnName : spawnNames) {
		...
		if (spawnName == "groundflash") {
			spawnName = "CStandardGroundFlash";
		...
		for (const auto& [propName, propValue]: props) {
			...
			if (CExpGenSpawnable::GetSpawnableMemberInfo(className, memberInfo)) {
				...
				LOG_L(L_WARNING, "[CCEG::%s] unknown field %s::%s in spawn-table \"%s\" for CEG \"%s\"", __func__, tag, propName.c_str(), spawnName.c_str(), className.c_str());
			}
		}
